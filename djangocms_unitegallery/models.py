...
class GalleryPhoto(models.Model):
    ...
    mp4video = models.TextField(
        _("MP4 video url"),
        blank=True,
        help_text=_(
            "Leave this blank if you don't want to display a video."
        )
    )
    youtubevideo = models.TextField(
        _("YouTube video id"),
        blank=True,
        help_text=_(
            "Leave this blank if you don't want to display a YouTube video."
        )
    )
    ...
    def check_if_video(self):
        return self.mp4video or self.youtubevideo

    def get_video_object_zh(self):
        if self.mp4video:
            return {'video': self.mp4video, 'type': 'html5'}
        elif self.youtubevideo:
            return {'video': self.youtubevideo, 'type': 'youtube'}

    def get_video_object_en(self):
